...
class HexapodParser:
    ...
    def _parser(self):
        ...
        for num_joints in range(p.getNumJoints(self.hexapod)):
            joint_info = p.getJointInfo(hexapod, num_joints)
            ...
            data = {
                    ...
                    "joint_center": (joint_lower_limit + joint_upper_limit) / 2
                    }
    ...
    def leg_move_order(self):
        ...
        leg_num_each_side = len(left_legs) if len(left_legs) == len(right_legs) else print("leg numbers aren't same")

        for leg_num in range(leg_num_each_side):
...
HexapodGait = HexapodWaveGait(HParser)
while 1:
    p.stepSimulation()
    current_time = time.time() - start_time
    HexapodGait.hexapod_wave_gait(current_time)
